...
export default function CommissionScope({ className = '' }: CommissionScopeProps) {
  ...
  const tabs = [
    {
      id: 'scope' as const,
      label: isEnglish ? 'Commission Scope' : 'কমিশনের কার্যক্ষেত্র',
      icon: (
    ...
    <div className={`bg-white dark:bg-slate-800 rounded-xl shadow-lg overflow-hidden ${className}`}>
      ...
      <div className="bg-gradient-to-r from-green-600 to-green-700 px-6 py-4">
        <h2 className="text-xl font-bold text-white flex items-center">
          ...
          {isEnglish ? 'Commission Scope & Powers' : 'কমিশনের কার্যক্ষেত্র ও ক্ষমতা'}
        </h2>
      </div>
      ...
      <div className="p-6">
        <div className="space-y-4">
          {getCurrentData().map((item, index) => (
            <div
              key={index}
              className="flex items-start space-x-3 p-4 bg-gray-50 dark:bg-slate-700/50 rounded-lg border-l-4 border-green-500"
            >
