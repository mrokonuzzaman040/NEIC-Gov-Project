...
export async function GET(request: NextRequest) {
  try {
    ...
    // Security: Only fetch necessary fields for management view
    const submissions = await prisma.submission.findMany({
      ...
      select: {
        ...
        attachmentType: true,
        attachmentUrl: true,
        ipHash: true // For security tracking
      },
      orderBy: {
        createdAt: 'desc'
      },
    ...
    const sanitizedSubmissions = submissions.map(submission => ({
      ...
      // Note: ipHash is excluded for security
    }));
    ...
    try {
      await prisma.userAuditLog.create({
        data: {
          ...
          action: 'SUBMISSIONS_ACCESS',
          details: JSON.stringify({
            endpoint: 'management/submissions',
      ...
      console.warn('Failed to log submissions access:', auditError);
    }
    ...
    console.error('Management submissions error:', error);
